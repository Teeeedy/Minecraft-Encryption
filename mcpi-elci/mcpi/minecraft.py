...
from .cryptography_session import CryptographySession
import math
...
class Minecraft:
    ...
    def __init__(self, connection):
        ...
        self.cryptographySession = CryptographySession()
        self.conn.setCryptoSession(self.cryptographySession)
        self.swapPublicKey()
        

        #Swap public keys on start


    #Swap public keys with java recipient
    def swapPublicKey(self):
        public_key = self.cryptographySession.public_key

        key_out = bytes.decode(public_key.exportKey()).replace("\n", "")
        recipientKey = self.conn.sendReceiveRaw(b"srv.swapPublicKey", key_out)

        self.cryptographySession.setRecipientPublicKey(recipientKey)

        if(self.cryptographySession.recipient_public_key is not None):
            print("Successfully constructed recipient public key")
