...
public class RemoteSession {
	...
	private final CryptographySession cryptographySession;
	...
	public RemoteSession(ELCIPlugin plugin, Socket socket) throws IOException {
		...
		this.cryptographySession = new CryptographySession();
		init();
	}
	...
	protected void handleLine(String line) {
		...
		//decrypt msg here
		boolean skipDecryption = false;
		if(line.contains("NO_ENCRYPTION:")) {
			line = line.replace("NO_ENCRYPTION:", "");
			skipDecryption = true;
		}

		if(!skipDecryption){
			if(!line.contains(",")){
				System.out.println("MESSAGE DOES NOT CONTAIN SIGNATURE - IGNORING MESSAGE");
				return;
			}
			...
			byte[] data = cryptographySession.decrypt(line.getBytes(charSet));
			line = new String(data, charSet);

			boolean verification_value = cryptographySession.verify(data, signature);

			if(!verification_value){
				System.out.println("SIGNATURE NOT VERIFIABLE - IGNORING MESSAGE");
				return;
			}
		}
...
//		System.out.println(line);
	...
	protected void handleCommand(String c, String[] args) {
		...
		try {
			...
			switch (c) {
				...
				case "srv.swapPublicKey": {
					//FORMAT = MODULUS,EXPONENT
					//SEND PUBLIC KEY
					{
						byte[] data = this.cryptographySession.publicKey.getEncoded();
						//Encode to base64
						data = Base64.getEncoder().encode(data);

						sendRaw(new String(data));
					}
					...
					//RECEIVE RECIPIENT PUBLIC KEY
					{
						...
						cryptographySession.setRecipientPublicKey(args[0].getBytes(charSet));

						if(cryptographySession.recipientPublicKey != null){
							System.out.println("Successfully constructed recipient public key");
						}
					}
					break;
				}
	...
	public void send(String a) {
		//TODO ENCRYPT
		PublicKey recipientPublicKey = cryptographySession.recipientPublicKey;
		if(recipientPublicKey == null){
			System.out.println("CANNOT SEND PACKET; PUBLIC KEY NOT GENERATED");
			return;
		}

		//CIPHERTEXT,HASH(PLAINTEXT)
		byte[] out = cryptographySession.encrypt(a.getBytes(charSet));
		byte[] signature = cryptographySession.sign(a.getBytes(charSet));
	

		String cipherText = new String(out, UTF_8);
		String signatureText = new String(signature, UTF_8);

		if (pendingRemoval) return;
		synchronized(outQueue) {
			outQueue.add(cipherText + "," + signatureText);
		}
	}

	public void sendRaw(Object a) {
		sendRaw(a.toString());
	}

	public void sendRaw(String a) {
		a = "NO_ENCRYPTION:" + a;
		if (pendingRemoval) return;
		synchronized(outQueue) {
			outQueue.add(a);
